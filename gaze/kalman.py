...
if __name__ == "__main__":
    ...
    for i in range(30):
        ...
        # 通过Q矩阵产生误差
        w = np.array(
            [[gaussian_distribution_generator(Q[0, 0])],
             # gaussian_distribution_generator(Q[0, 0])的值为0.01103097596，（这值是随机的，每次运行都不一样）
             [gaussian_distribution_generator(
                 Q[1, 1])]])  # gaussian_distribution_generator(Q[1, 1])的值为-0.1242726240，（这值是随机的，每次运行都不一样）
        ...
        ##通过R矩阵产生误差
        v = np.array(
            [[gaussian_distribution_generator(R[0, 0])],
             # gaussian_distribution_generator(R[0, 0])的值为-0.62251186549，（这值是随机的，每次运行都不一样）
             [gaussian_distribution_generator(
                 R[1, 1])]])  # gaussian_distribution_generator(R[1, 1])的值为2.52779100481，（这值是随机的，每次运行都不一样）
